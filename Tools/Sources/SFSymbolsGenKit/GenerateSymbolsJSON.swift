...
fileprivate func getClipboardContents() -> String? {
    return NSPasteboard.general.string(forType: .string)
}

/// Loads draw category symbols from draw.txt or clipboard
fileprivate func loadDrawCategorySymbols(drawFilePath: URL) -> Set<String> {
...
fileprivate enum Localization: String, CaseIterable, Codable {
...
fileprivate func detectLocalization(from symbolName: String) -> (baseName: String, localization: Localization?) {
...
fileprivate struct SFCategoryInfo: Codable {
    let icon: String
    let title: String
}
...
fileprivate struct ScannedSymbol {
    var name: String
    var year: String?
    var layersets: [String]
}

fileprivate struct MergedSymbol {
    var baseName: String
    var baseSymbol: ScannedSymbol?
    var localizedVariants: [(localization: Localization, symbol: ScannedSymbol)]
}

// MARK: - Category info from categories.plist

fileprivate struct CategoryPlistEntry: Codable {
    let key: String
    let label: String
    let icon: String
...
/// Reads CoreGlyphs + SF Symbols app metadata, merges, applies `draw.txt` and
/// `font_restrictions.tsv` from `workingDir`, and writes `symbols.json` into
/// `workingDir`.
///
/// Throws `GenError` on a fatal failure (matching the original script's `exit(1)`
/// paths).
func generateSymbolsJSON(appPath: String, workingDir: URL) throws {
    let inputURL: URL = {
    ...
    guard FileManager.default.fileExists(atPath: inputURL.path) else {
        throw GenError.missing("Error: Could not find Metadata folder at expected path: \(inputURL.path)")
    }

    let drawFilePath = workingDir.appendingPathComponent("draw.txt")
    ...
    guard let nameData = try? Data(contentsOf: coreGlyphsNameAvailabilityURL),
        ...
        throw GenError.missing("Error: Could not read name_availability.plist from CoreGlyphs")
    }
    ...
    // per line.
    let fontRestrictionsURL = workingDir.appendingPathComponent("font_restrictions.tsv")
    ...
    let drawCategorySymbols = loadDrawCategorySymbols(drawFilePath: drawFilePath)
    ...
    guard let jsonData = try? encoder.encode(output) else {
        throw GenError.missing("Error: Could not encode JSON")
    }

    let outputURL = workingDir.appendingPathComponent("symbols.json")

    do {
        try jsonData.write(to: outputURL)
        print("Generated symbols.json successfully.")
    } catch {
        throw GenError.missing("Error writing file: \(error)")
    }
