...
public extension UIBarButtonItem {
    convenience init(symbol: SFSymbol, landscapeImagePhone: UIImage?, style: UIBarButtonItem.Style, target: Any?, action: Selector?){
        self.init(image: UIImage(symbol: symbol), landscapeImagePhone: landscapeImagePhone, style: style, target: target, action: action)
    }

    convenience init(symbol: SFSymbol, style: UIBarButtonItem.Style, target: Any?, action: Selector?){
        self.init(image: UIImage(symbol: symbol), style: style, target: target, action: action)

    }
    
    @available(iOS 14.0, *)
    convenience init(title: String? = nil, symbol: SFSymbol, primaryAction: UIAction? = nil, menu: UIMenu? = nil){
